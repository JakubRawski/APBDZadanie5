...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        var emps = Database.GetEmps();
         ...
         var result = (from e in emps group e by e.DeptNo into g select
             new {DeptNo = g.Key, Count = g.Count()}).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var grades = Database.GetSalgrades();
         ...
         var result = (from e in emps from s in grades where
              e.Sal <= s.Hisal && e.Sal >= s.Losal select new {e.EName,s.Grade}).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result =  (from e in emps join avg in 
                (from e in emps
                group e by e.DeptNo into g
                select new { DeptNo = g.Key, AvgSal = g.Average(e => e.Sal) })
        on e.DeptNo equals avg.DeptNo where e.Sal > avg.AvgSal select e.EName).ToList();; 
